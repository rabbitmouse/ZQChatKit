...
@interface ZQAudioPlayer()<AVAudioPlayerDelegate>
@end

@implementation ZQAudioPlayer

+ (ZQAudioPlayer *)sharedInstance
{
    static ZQAudioPlayer *sharedInstance = nil;
...
- (void)playSongWithUrl:(NSString *)songUrl
{
    dispatch_async(dispatch_queue_create("playSoundFromUrl", NULL), ^{
        [self.delegate ZQAudioPlayerBeiginLoadVoice];
...
- (void)playSoundWithData:(NSData *)soundData
{
    ...
    [_player play];
    [self.delegate ZQAudioPlayerBeiginPlay];
}
...
{
    [self.delegate ZQAudioPlayerDidFinishPlay];
}
